...
void conv2d_hls( TwoDTile_IPT x[],  TwoD_filterT Kernel_i[], float z[],  float b, TwoD_IPT X[])
{
...
//#pragma HLS UNROLL factor=2
#pragma HLS data_pack variable=w_set
						//if(col > 0)
						{
			...
			/* currZ_bigTile = */ conv2d_hls(x_row, w_set[0].ith_filter, z_ , b[i], X);
			for( int u = 0; u < outDim; u++)
			{
				Z[i].Z_out[readImgRow][u] = z_[u];
			}
